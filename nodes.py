...
class LP_Engine:
    ...
    def load_model(self, model_config, model_type):
        ...
        if model_type == "stitching_retargeting_module":
            ckpt_path = os.path.join(
                get_model_dir("liveportrait"), "retargeting_models", model_type + ".pth"
            )
        else:
            ckpt_path = os.path.join(
                get_model_dir("liveportrait"), "base_models", model_type + ".pth"
            )
        ...
        if os.path.isfile(ckpt_path) == False:
            is_safetensors = True
            ckpt_path = os.path.join(
                get_model_dir("liveportrait"), model_type + ".safetensors"
            )
            if os.path.isfile(ckpt_path) == False:
                self.download_model(
                    ckpt_path,
                    "https://huggingface.co/Kijai/LivePortrait_safetensors/resolve/main/"
                    + model_type
                    + ".safetensors",
                )
        ...
        if model_type == "appearance_feature_extractor":
            ...
            if is_safetensors:
                stitcher.load_state_dict(
                    self.filter_for_model(checkpoint, "retarget_shoulder")
                )
            else:
                stitcher.load_state_dict(
                    self.remove_ddp_dumplicate_key(checkpoint["retarget_shoulder"])
                )
    ...
    def load_models(self):
        ...
        model_config_path = os.path.join(
            current_directory, "LivePortrait", "config", "models.yaml"
        )
        model_config = yaml.safe_load(open(model_config_path, "r"))

        appearance_feature_extractor = self.load_model(
            model_config, "appearance_feature_extractor"
        )
        ...
        stitching_retargeting_module = self.load_model(
            model_config, "stitching_retargeting_module"
        )
    ...
    def expand_img(self, rgb_img, square):
        ...
        crop_trans_m = create_transform_matrix(
            max(-square[0], 0), max(-square[1], 0), 1, 1
        )
    ...
    def prepare_src_image(self, img):
        ...
        if h != input_shape[0] or w != input_shape[1]:
            ...
            x = cv2.resize(
                img, (input_shape[0], input_shape[1]), interpolation=interpolation
            )
        else:
            x = img.copy()
    ...
    def GetMaskImg(self):
        if self.mask_img is None:
            path = os.path.join(
                ...
                "./LivePortrait/utils/resources/mask_template.png",
            )
    ...
    def crop_face(self, img_rgb, crop_factor):
        ...
        face_region, is_changed = self.calc_face_region(
            crop_region, get_rgb_size(img_rgb)
        )
    ...
    def prepare_source(self, source_image, crop_factor, is_video=False, tracking=False):
        ...
        for img_rgb in source_image_np:
            if tracking or len(psi_list) == 0:
                ...
                crop_trans_m = create_transform_matrix(
                    crop_region[0], crop_region[1], s_x, s_y
                )
                mask_ori = cv2.warpAffine(
                    self.GetMaskImg(),
                    crop_trans_m,
                    get_rgb_size(img_rgb),
                    cv2.INTER_LINEAR,
                )
                ...
                if is_changed:
                    ...
                    crop_trans_m = create_transform_matrix(
                        crop_region[0], crop_region[1], s, s
                    )
...
class PrintExpData:
    ...
    def run(self, cut_noise, exp=None):
        ...
        print(
            f"sorted_list: {[[item[2], round(float(item[1]),1)] for item in sorted_list]}"
        )
        return (exp,)
        ...
        for line in lines:
            ...
            try:
                ...
                if idx == 0:
                    ...
                    es = ExpressionSet(es=motoin_link[idx])
                cmds = cmds[1].split(":")
                change = int(cmds[0])
                keep = int(cmds[1])
            except:
    ...
    def run(
        ...
        if src_images != None:
            ...
            if id(src_images) != id(self.src_images) or self.crop_factor != crop_factor:
                ...
                if 1 < src_length:
                    self.psi_list = g_engine.prepare_source(
                        src_images, crop_factor, True, tracking_src_vid
                    )
                else:
        ...
        for i in range(total_length):
            ...
            if i < driving_length:
                ...
                if d_0_es is None:
                    d_0_es = ExpressionSet(
                        erst=(d_i_info["exp"], d_i_r, d_i_info["scale"], d_i_info["t"])
                    )
            ...
            self.pbar.update_absolute(
                i + 1, total_length, ("PNG", Image.fromarray(crop_out), None)
            )
    ...
    def run(
        ...
        if sample_image != None:
            ...
            if sample_parts == "OnlyRotation" or sample_parts == "All":
                ...
                retargeting(
                    es.e, self.d_info["exp"], sample_ratio, (1, 2, 11, 13, 15, 16)
                )
        ...
        img.save(
            os.path.join(folder_paths.get_temp_directory(), filename), compress_level=1
        )
        results = list()
